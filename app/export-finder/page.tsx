...
const BAND_STYLES = {
  ...
  Competitive: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',
}
...
function GrowthBadge({ pct }: { pct: number }) {
  const pos = pct >= 0
  return (
    <span className={`text-xs font-bold ${pos ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-14">
      <nav className="mb-6 text-sm text-slate-500 dark:text-slate-400">
        ...
        <span className="mx-2">/</span>
        <span className="text-slate-700 dark:text-slate-300">Export Finder</span>
      </nav>

      <header className="mb-8">
        <p className="text-xs font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-400">Trade Intelligence</p>
        <h1 className="mt-2 text-3xl sm:text-4xl font-bold text-slate-900 dark:text-slate-100">Export Opportunity Finder</h1>
        <p className="mt-2 max-w-2xl text-slate-500 dark:text-slate-400">
          ...
          Source: DGFT, APEDA, Ministry of Commerce.
        </p>
      </header>
      ...
      <div className="mb-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
            ...
            <p className="mt-1 text-xl font-bold text-slate-900 dark:text-slate-100">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        ...
        <select value={access} onChange={e => setAccess(e.target.value)} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500">
        ...
        <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500">
        ...
        {(sector || access || search) && (
          <button onClick={() => { setSector(''); setAccess(''); setSearch('') }} className="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm text-slate-500 hover:text-slate-800 dark:hover:text-slate-200">
            Clear ✕
          </button>
        )}
      </div>
      ...
      <div className="space-y-3">
        {filtered.map(cat => {
          ...
          return (
            <div key={cat.id} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
              <button
                onClick={() => setExpanded(isOpen ? null : cat.id)}
                className="w-full flex flex-wrap sm:flex-nowrap items-center gap-4 p-5 text-left hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
              >
                ...
                <div className="shrink-0 w-12 h-12 rounded-full flex items-center justify-center border-2 border-current"
                  style={{ borderColor: cat.opportunityScore >= 85 ? '#10b981' : cat.opportunityScore >= 72 ? '#3b82f6' : cat.opportunityScore >= 60 ? '#f59e0b' : '#94a3b8' }}>
                  <span className="text-xs font-bold" style={{ color: cat.opportunityScore >= 85 ? '#10b981' : cat.opportunityScore >= 72 ? '#3b82f6' : cat.opportunityScore >= 60 ? '#f59e0b' : '#94a3b8' }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-400">HS {cat.hsCode}</span>
                  </div>
                  <div className="flex flex-wrap items-center gap-3 mt-1">
                    <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">${cat.exportValueUSD.toFixed(2)}B FY24</span>
                ...
                <div className="hidden sm:flex items-center gap-1.5 shrink-0">
                  {cat.topMarkets.slice(0, 3).map(m => (
                    <span key={m.country} className="text-[10px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 rounded px-1.5 py-0.5">{m.country} {m.sharePct}%</span>
                  ))}
                </div>

                <span className="text-slate-400 ml-2 shrink-0">{isOpen ? '▲' : '▼'}</span>
              </button>

              {isOpen && (
                <div className="px-5 pb-6 pt-0 border-t border-slate-100 dark:border-slate-800">
                  <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-5 mb-5">
                    {[
                      ...
                      <div key={m.label} className="rounded-xl bg-slate-50 dark:bg-slate-800 p-3">
                        <p className="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500">{m.label}</p>
                        <p className="mt-1 text-base font-bold text-slate-800 dark:text-slate-200">{m.value}</p>
                      </div>
                    ))}
                  </div>

                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-4">{cat.overview}</p>

                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-600 mb-2">Top Export Markets</p>
                      <div className="space-y-1.5">
                        {cat.topMarkets.map(m => (
                          <div key={m.country} className="flex items-center gap-2">
                            <span className="w-24 text-xs text-slate-500 dark:text-slate-400 shrink-0">{m.country}</span>
                            <div className="flex-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800">
                              <div className="h-full rounded-full bg-indigo-400 dark:bg-indigo-500" style={{ width: `${(m.sharePct / cat.topMarkets[0].sharePct) * 100}%` }} />
                            </div>
                            <span className="text-xs text-slate-400 w-8 text-right">{m.sharePct}%</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-600 mb-1.5">Key Production States</p>
                        <div className="flex flex-wrap gap-1.5">
                          {cat.keyStates.map(s => (
                            <span key={s} className="text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded px-2 py-0.5">{s}</span>
                          ))}
                        </div>
                      </div>
                      <div>
                        <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-600 mb-1.5">Certifications Needed</p>
                        <div className="flex flex-wrap gap-1.5">
                          {cat.certifications.slice(0, 4).map(c => (
                            <span key={c} className="text-xs bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 rounded px-2 py-0.5">{c}</span>
                          ))}
                        </div>
                      </div>
                      <div>
                        <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-600 mb-1.5">Min Entry Order</p>
                        <p className="text-sm text-slate-700 dark:text-slate-300 font-medium">{cat.minOrderValue}</p>
                      </div>
                    </div>
                  </div>

                  <div className="mt-4">
                    <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-600 mb-2">Tailwinds</p>
                  ...
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-3">Seasonality: {cat.seasonality}</p>
                </div>
              )}
            </div>
          )
